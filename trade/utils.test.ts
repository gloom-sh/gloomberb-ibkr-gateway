...
describe("trade-utils", () => {
  ...
  test("resolves order tickers by direct symbol or broker contract metadata", () => {
    ...
    expect(findTickerForOrder({
      contract: { symbol: "AMD", brokerInstanceId: "ibkr-paper", brokerId: "ibkr" },
    }, tickers)?.metadata.ticker).toBe("AMD");
    expect(findTickerForOrder({
      contract: { symbol: "SPY", localSymbol: "SPY  250117C00600000", conId: 123, brokerInstanceId: "ibkr-paper", brokerId: "ibkr" },
